...
amberbockel/ui uses CSS custom properties for theming, with dark mode as the default. Theme switching works via the `data-theme` attribute on `<html>`.
...
```tsx
import { useTheme, Nav } from 'amberbockel/ui'
...
```tsx
import { colors, typography, spacing, motion, effects } from 'amberbockel/ui'
