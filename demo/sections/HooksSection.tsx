  ...
  return (
    <button onClick={toggleTheme}>
      ...
      <DemoBlock title="useMediaQuery / useIsMobile / useBreakpoint">
        <div className="p-6 rounded-[32px] border border-[var(--glass-border)] bg-[var(--glass-bg)] space-y-3">
          <div className="flex flex-wrap gap-2">
            <Badge variant={isMobile ? 'lavender' : 'outline'}>Mobile: {isMobile ? 'Yes' : 'No'}</Badge>
            <Badge variant={isTablet ? 'lavender' : 'outline'}>Tablet: {isTablet ? 'Yes' : 'No'}</Badge>
            <Badge variant={isDesktop ? 'lavender' : 'outline'}>Desktop: {isDesktop ? 'Yes' : 'No'}</Badge>
          </div>
      ...
      <DemoBlock title="useReducedMotion">
        <div className="p-6 rounded-[32px] border border-[var(--glass-border)] bg-[var(--glass-bg)]">
          <div className="flex items-center gap-3">
            ...
            <Badge variant={prefersReducedMotion ? 'pink' : 'lavender'}>
...
function SearchDialog() {
  ...
  return <span>Press {modKey}+K to search</span>
      ...
      <DemoBlock title="Animation Hooks">
        <div className="p-6 rounded-[32px] border border-[var(--glass-border)] bg-[var(--glass-bg)] space-y-4">
          <div>
