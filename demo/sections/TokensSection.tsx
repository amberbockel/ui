...
const accentTokens = [
  { name: '--color-periwinkle', value: 'var(--color-periwinkle)', label: 'Periwinkle' },
  { name: '--color-pink', value: 'var(--color-pink)', label: 'Pink' },
  { name: '--color-lavender', value: 'var(--color-lavender)', label: 'Lavender' },
  { name: '--color-green', value: 'var(--color-green)', label: 'Green' },
]
    ...
    <DemoSection id="tokens" title="Design Tokens" description="CSS custom properties that power the design system. Automatically adapt to light/dark themes.">
      ...
      <DemoBlock title="Glass Morphism">
        ...
        <div className="flex gap-4 mt-6">
          <div className="flex-1 p-6 rounded-[32px] bg-[var(--glass-bg)] border border-[var(--glass-border)] backdrop-blur-lg">
            <p className="text-sm text-[var(--color-grey-400)]">Glass panel with blur</p>
          </div>
          <div className="flex-1 p-6 rounded-[32px] bg-[var(--glass-bg)] border border-[var(--glass-highlight)]">
      ...
      <DemoBlock title="Typography">
        <div className="space-y-3">
          <p className="font-display text-4xl font-semibold tracking-[0.05em] text-[var(--color-white)]">Prata (Display)</p>
          <p className="font-sans text-lg text-[var(--color-white)]">DM Sans (Body/UI & Code)</p>
        </div>
      </DemoBlock>
