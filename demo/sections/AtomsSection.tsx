...
export function AtomsSection() {
  ...
  const [badgeVariant, setBadgeVariant] = useState<'default' | 'lavender' | 'pink' | 'indigo' | 'green' | 'outline'>('default')
  ...
  return (
    <DemoSection id="atoms" title="Atoms" description="The fundamental design tokens for Amber's UI system.">
      {/* Buttons */}
      <DemoBlock title="Button">
        <div className="space-y-6">
          ...
          {/* Preview */}
          <div className="p-8 rounded-[32px] border border-[var(--glass-border)] bg-[var(--glass-bg)] flex items-center justify-center gap-4">
  ...
  Button
      ...
      <DemoBlock title="Badge">
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-xs text-[var(--color-grey-400)] mr-2">Variant:</span>
            {(['default', 'lavender', 'pink', 'indigo', 'green', 'outline'] as const).map((v) => (
              <button
                key={v}
          ...
          {/* Preview */}
          <div className="p-8 rounded-[32px] border border-[var(--glass-border)] bg-[var(--glass-bg)] flex flex-wrap gap-3 items-center justify-center">
          ...
          <div className="p-8 rounded-[32px] border border-[var(--glass-border)] bg-[var(--glass-bg)] flex items-center justify-center">
            <div className="w-full max-w-sm">
              <Input
                ...
                inputSize={inputSize}
                placeholder={inputError ? 'Invalid topic' : 'Enter your mentoring goals...'}
                value={inputValue}
                onChange={(e) => setInputValue(e.target.value)}
                leftIcon={inputIcon ? <Icon name="search" size="sm" /> : undefined}
                error={inputError ? 'Please enter a valid topic' : undefined}
              />
            </div>
          </div>
      ...
      <DemoBlock title="Icon (Iconoir)">
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {([
              { label: 'default', value: '' },
              { label: 'lavender', value: 'var(--color-lavender)' },
              { label: 'pink', value: 'var(--color-pink)' },
              { label: 'indigo', value: 'var(--color-indigo)' },
